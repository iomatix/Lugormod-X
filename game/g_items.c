...
int MaxAmmo(gentity_t *ent, int ammo) {
	...
	{
		if (PlayerAcc_Prof_GetProfession(ent) == PROF_JEDI) return  ammoData[ammo].max - floor((ammoData[ammo].max / 1.65)) + floor(ammoData[ammo].max * PlayerAcc_Prof_GetLevel(ent)* 0.021);
	}


	return ammoData[ammo].max + (ammoData[ammo].max * PlayerProf_Merc_GetAmmoSkill(ent) * 0.2) + floor(ammoData[ammo].max * PlayerAcc_Prof_GetLevel(ent)* 0.028);
}
//======================================================================
...
void Touch_Item(gentity_t *ent, gentity_t *other, trace_t *trace) {
	...
	{
		if (other->client->pers.Lmd.account) //iomatix if is logged in
		{
			if (PlayerAcc_Prof_GetLevel(other) < 40 || PlayerAcc_Prof_GetProfession(other) != PROF_JEDI) //iomatix skip for jedi's duo specialization above mastery level which is 40.
			{
